...
Work proceeds in volumes (see `docs/ROADMAP.md`). Volumes 0–6 are complete
(foundation, data layer, feature engineering, quantile forecasting, newsvendor
optimization, historical policy simulation, executive reporting); Volume 7
(Streamlit dashboard) is next.
...
- `src/demandpilot/` — layered package: `core/` (pure math) → `config/` → `data/` →
  `features/` → `forecasting/` → `optimization/` → `simulation/` → `reporting/` → `cli.py`
