...
| 2026-07-12 | Report templates ship as package data under `demandpilot/reporting/templates/`, not a `reports/templates/` runtime path | Honors the Volume 0 `.gitignore` comment ("templates are code and live in src/") rather than contradicting it |
| 2026-07-12 | Report's "largest misses" table capped at 20 rows, with the total count stated alongside | Real M5 scale (30k+ series) makes a full dump useless for an executive audience; the cap is visible, never silent |
